...
jQuery(document).ready( function($) {
	...
	$('#LoginWithAjax_Register form').submit(function(event){
		...
		$.post(url, postData, function(data){
			...
			lwaAjax( data, 'LoginWithAjax_Register_Status', '.register-form-inner' );
		}, "json");
		$.ajax({
		  type: 'POST',
		  url: url,
	...
	$('#LoginWithAjax_Links_Remember').click(function(event){
		event.preventDefault();
		$('#LoginWithAjax_Remember').show(300);
	});
	$('#LoginWithAjax_Links_Remember_Cancel').click(function(event){
		event.preventDefault();
		$('#LoginWithAjax_Remember').hide(300);
	});
		...
		if( data.result === true || data.result === false ){
			if(data.result === true){
				...
				if( $('#'+statusElement).length > 0 ){
					$('#'+statusElement).attr('class','alert alert-success').html(data.message);
				}else{
					$('<span id="'+statusElement+'" class="alert alert-success">'+data.message+'</span>').prependTo( prependTo );
				}
			}else{
				//Login Failed
				//If there already is an error element, replace text contents, otherwise create a new one and insert it
				if( $('#'+statusElement).length > 0 ){
					$('#'+statusElement).attr('class','alert alert-error').html(data.error);
				}else{
					$('<span id="'+statusElement+'" class="alert alert-error">'+data.error+'</span>').prependTo( prependTo );
				}
			...
			//If there already is an error element, replace text contents, otherwise create a new one and insert it
			if( $('#'+statusElement).length > 0 ){
				$('#'+statusElement).attr('class','alert alert-error').html('An error has occured. Please try again.');
			}else{
				$('<span id="'+statusElement+'" class="alert alert-error">An error has occured. Please try again.</span>').prependTo( prependTo );
			}
		}
	}
